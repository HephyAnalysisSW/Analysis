...
KinematicReconstruction_LSroutines::KinematicReconstruction_LSroutines(const float& mass_Wp, const float& mass_Wm)
{
    mt_    = TopMASS;
    mtbar_ = TopMASS;
    mb_    = 4.8;
...
KinematicReconstruction_LSroutines::KinematicReconstruction_LSroutines(const float& mass_top, const float& mass_topbar,
                                                                       const float& mass_b, const float& mass_bbar,
                                                                       const float& mass_Wp, const float& mass_Wm,
...
void KinematicReconstruction_LSroutines::ini(const float& mass_Wp, const float& mass_Wm)
{
    mt_    = TopMASS;
    mtbar_ = TopMASS;
    mb_    = 4.8;
...
void KinematicReconstruction_LSroutines::setConstraints(const TLorentzVector& LV_al, const TLorentzVector& LV_l, const TLorentzVector& LV_b, const TLorentzVector& LV_bbar, const float& missPx, const float& missPy)
{
    l_  = LV_l;
    al_ = LV_al;
    b_  = LV_b;
...
void KinematicReconstruction_LSroutines::setConstraints(const LV& LV_al, const LV& LV_l, const LV& LV_b, const LV& LV_bbar, const float& missPx, const float& missPy)
{
...
float KinematicReconstruction_LSroutines::landau2D(const float& xv, const float& xvbar)const
{
...
void KinematicReconstruction_LSroutines::doAll()
{
        ...
        nSol_=vect_pxv_[0];
            ...
            for(int i=1;i<=nSol_;++i)
            {
                ...
                //proton Energy [GeV]
                //FIXME:Warning , hardcoded value of proton Energy [GeV].
                float protonE = 6500; //FIXME:  set as global variable
...
void KinematicReconstruction_LSroutines::topRec(const float& px_neutrino)
{
    float pxp, pyp, pzp, pup, pvp, pwp;
...
void KinematicReconstruction_LSroutines::findCoeff(float* const koeficienty)
{
...
float KinematicReconstruction_LSroutines::sqr(const float& x)const
{
    return (x*x);
}
...
void KinematicReconstruction_LSroutines::swap(float& realone, float& realtwo)const
{
    if(realtwo < realone){
        float aux = realtwo;
        realtwo = realone;
...
void KinematicReconstruction_LSroutines::quartic_equation(const float& h0, const float& h1, const float& h2, const float& h3, const float& h4, std::vector<float>& v)const
{
           ...
           else
            {
                ...
                else
                {
                   //printf("else3\n"); //printout
                     ...
                     float H3=h3/h0;
                     float H4=h4/h0;
                   float K1 = H2 -3*sqr(H1)/8;
                   float K2 = H3 + H1*sqr(H1)/8-H1*H2/2;
                   float K3 = H4-3*sqr(sqr(H1))/256+sqr(H1)*H2/16-H1*H3/4;
                    ...
                    {
                        //printf("else4\n"); //printout
                        std::vector<float> result_t12;

                        std::vector<float> result_t1;
                            result_t1.push_back(0);
                        std::vector<float> result_t2;
...
void KinematicReconstruction_LSroutines::cubic_equation(const float& a, const float& b, const float& c, const float& d, std::vector<float>& v)const
{
    ...
    else
    {
       ...
       float q = (s1*s1-3*s2)/9;
       float q3 = q*q*q;
       float r = (2*s1*s1*s1-9*s1*s2+27*s3)/54;
       float r2 = r*r;
       float S = r2-q3;
       if(sign(S)<0)
       {
...
void KinematicReconstruction_LSroutines::quadratic_equation(const float& a, const float& b, const float& c, std::vector<float>& v)const
{
     std::vector<float> result;
     //printf("a: %10.10f\n",a);//printout
    if(a==0)
...
void KinematicReconstruction_LSroutines::linear_equation(const float& a, const float& b, std::vector<float>& v)const
{
    std::vector<float> result;
    if(a==0)
    {
